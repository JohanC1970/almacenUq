...
public class DetalleVentaController implements Initializable{
    	...
    	txtCantidad.textProperty().addListener((observable, oldValue, newValue) -> {
    	    if (!tablaProductos.getSelectionModel().isEmpty() && !newValue.isEmpty()) {
    	        try {
    	            ...
    	            System.out.println("Error: El valor de cantidad no es un número válido.");
    	        }
    	    }
    	});
    	...
    	if(productoSeleccionado!= null){
    		if(!(txtCantidad.equals("") || txtCantidad ==null)){
    			...
    			if(productoSeleccionado.getCantidadExistencia()>= cantidad){
        			...
        			mostrarMensaje("Gestión detalle venta", "Proceso incompleto", "No hay unidades disponibles para realizar la venta", AlertType.INFORMATION);
    			}
    		}else{
    ...
    private void agregarProductoCarrito(Producto productoSeleccionado, int cantidad) throws ProductoException {
    	...
    	if(mfm.comprarProducto(ventaCompletada, productoSeleccionado, cantidad)){
			mostrarMensaje("Gestión detalle venta", "Proceso existoso", "Se agrego al carrito", AlertType.CONFIRMATION);
			...
			mostrarMensaje("Gestión detalle venta", "Proceso incompleto", "No hay unidades suficientes", AlertType.INFORMATION);
    	}
	}
